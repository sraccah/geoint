...
export function TopBar() {
    ...
    useEffect(() => {
        const update = () => {
            setTime(new Date().toISOString().replace('T', ' ').substring(0, 19) + 'Z');
        };
        update();
    ...
    return (
        <div className="hud-panel border-b border-hud-border flex items-center gap-3 px-3 py-2 z-20 shrink-0">
                ...
                <button
                    ...
                    className={cn(
                        ...
                        leftPanelOpen
                            ...
                            : 'border-hud-border text-hud-text-dim hover:border-hud-cyan hover:text-hud-cyan'
                    )}
                >
                    <PanelLeft size={13} />
                </button>
            ...
            <div className="flex items-center gap-4 flex-1 min-w-0">
                <SearchBar />
                <div className="hidden xl:flex items-center gap-5 font-mono text-xs shrink-0">
                    ...
                    <StatBadge label="MILITARY" value={stats?.military ?? '—'} color="red" />
...
function StatBadge({ label, value, color }: {
    ...
    color: 'cyan' | 'amber' | 'red' | 'green' | 'dim';
}) {
    const colorClass = { cyan: 'text-hud-cyan', amber: 'text-hud-amber', red: 'text-hud-red', green: 'text-hud-green', dim: 'text-hud-text-dim' }[color];
    return (
